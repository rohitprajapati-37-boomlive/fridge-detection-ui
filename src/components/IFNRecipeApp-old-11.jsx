...
const ItemCard = ({ recipe, index = 0, selectedIngredients = [] }) => {
  ...
  <div className="mb-3">
    ...
    <div className="flex flex-wrap gap-2">
      {(showAllIngredients ? normalizedIngredients : normalizedIngredients.slice(0, 4))
        .map((ingredient, idx) => {
...
const IFNRecipeApp = () => {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br  via-red-50 ">
      <div className="p-6 max-w-6xl mx-auto bg-white rounded-2xl shadow-xl p-8 border border-gray-100">
        ...
        <div className=" bg-white p-8  mb-8">
          ...
          {/* <button
            onClick={async () => {
              try {
                const response = await fetch("https://ifn.coolify.vps.boomlive.in/find_recipe?ingredients=chicken&ingredients=tomatoes");
                const data = await response.json();
            ...
            className="w-full mt-3 p-3 bg-blue-500 text-white rounded-xl hover:bg-blue-600 transition-colors text-sm"
          >
            🧪 Test API Response (Check Console)
          </button> */}
        </div>
      </div>
    </div>
